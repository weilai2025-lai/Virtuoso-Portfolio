...
當 **source-to-body 電壓 Vsb 很小**（接近正常操作點）時，可在該操作點附近做一階近似，得到線性模型：
...
此模型不引入新物理，只是將原本的平方根關係在小訊號範圍內簡化，方便電路分析與設計估算。

---

### 26.10 體效應係數 γ 中電荷常數 q 的角色
...
### 26.11 Reverse Body Bias（RBB）— NMOS
...
### 26.12 Forward Body Bias（FBB）— NMOS
...
#### 物理效果
- Source–Body PN junction 反向偏壓減小
- 耗盡區變窄
- 通道更容易形成
...
- 原始體效應模型為平方根形式
- 小 Vsb 範圍內可用線性模型近似
- kγ 提供 Vt 對 Vsb 的直觀敏感度
- q 是將摻雜濃度轉換為電荷影響的基本尺度
- RBB / FBB 提供一種 **post-silicon 可調整 Vt 的手段**
  - 省電 ↔ 效能 的權衡

（NMOS 與 PMOS 體效應差異將於後續一併補入本章）
