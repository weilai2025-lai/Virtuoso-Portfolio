...
- t：載子穿越通道的時間 → **電子從 source 走到 drain 需要多久**
- L：通道長度
- v：載子速度
...
- $E = V_{ds}/L$ ：通道中的**平均橫向推力（電場）**
- $v = \mu E$ ：電子在材料中實際移動的速度（低電場假設）
- $t = L/v$ ：電子穿越整條通道所需的時間
...
- 通道電荷 **正比於**
  - gate 對 channel 的平均有效控制電壓 $V_{gs} - \frac{V_{ds}}{2}$
  - 再扣掉剛好形成反轉層所需的門檻電壓 $V_{t}$

其中：
- $-\frac{V_{ds}}{2}$ 代表 **因為 drain 電壓拉高，使得通道電位平均被抬高，gate 對通道的平均控制力變弱，能維持的反轉電荷因此變少。**
...
- 電流的本質： $I_{ds} = \frac{Q_{\text{channel}}}{t}$
- 線性區電流同時受到兩個 $V_{ds}$ 效應影響：
  - 括號外的 $V_{ds}$ ：推動載子移動（速度變快）
  - 括號內的 $-V_{ds}/2$ ：平均通道電荷變少
- 在線性區中，電流仍隨 $V_{ds}$ 增加，
...
- gate 無法再對那一段施加控制
- 通道中可被 gate 控制、可被注入的反轉電荷數量 **完全由 source 端決定**
...
$$
\begin{aligned}
I_{ds} &= \beta \left( V_{gs} - V_t - \frac{V_{gs} - V_t}{2} \right) (V_{gs} - V_t) \\
&= \beta \left( \frac{V_{gs} - V_t}{2} \right) (V_{gs} - V_t) \\
...
- $V_{gs}$ ：決定 **通道能不能存在、以及有多少反轉電荷**
- $V_{ds}$ （小）：提供橫向電場，推動載子（Linear）
- $V_{ds}$ （大）：夾斷 drain 端 channel，電流進入飽和（Saturation）
...
- NMOS：載子為電子（electrons），遷移率為 $\mu_n$
- PMOS：載子為電洞（holes），遷移率為 $\mu_p$
...
> 因此：
>
> - Source–Body 與 Drain–Body 之間 **天然形成 n–p 接面（PN junction）**
...
- **對導通能力（drive strength）來說**：大 $C_{ox}$ 有助於形成更多通道電荷 → $I_{ds}$ 大
- **對切換速度與功耗來說**：大電容意味著：需要更多能量充放電、需要更長時間完成電壓切換
...
- Gate–Channel 總電容為： $C_g = C_{ox}WL$
- 在電路模型中，Gate 電容需拆分為： $C_g = C_{gs} + C_{gd}$
...
- Source–Body 與 Drain–Body 之間 **天然形成 PN 接面**
- 在正常操作下，這些 PN 接面多半處於 **反向偏壓**
...
- 與 **邏輯功能無關**
- 只會：增加節點負載、降低切換速度、增加動態功耗（charging / discharging）
...
>
> - 若中間節點：需要接到其他電路、需要被觀察、回授或控制、在電路功能上仍然有意義
>
>   那麼即使擴散區可以共用，
>   **仍必須保留 via 將該節點拉到金屬層**，
>   這種情況就稱為 **shared diffusion**。
>
> - 若中間節點：只是一段導通路徑的一部分、不輸出、不回授、不被使用
>
...
- 中間那塊 diffusion：沒有固定身分，它只是「兩顆 NMOS 共用的內部節點」
- 當 NMOS1 導通時：它是 NMOS1 的 drain
- 當 NMOS2 導通時：它是 NMOS2 的 source
...
- 它是 **寄生電容**，對效能不利
- 電容大小與 **擴散區的面積與周長** 有關
- **共享 / 合併 diffusion** 是為了：實現邏輯功能、同時減少 diffusion capacitance
- 串接 NMOS 中間節點：不是固定的 source 或 drain，而是依電壓動態決定角色的內部節點
...
- **垂直電場（vertical field）** ：由 gate 電壓 $V_{gs}$ 造成，影響電子被壓到 Si–SiO₂ 界面的程度
- **橫向電場（lateral field）** ：由 $V_{ds}/L$ 造成，直接推動電子沿著通道移動
...
- 增加 $V_{gs}$ 會吸引 **更多電子** 到通道（反轉層），這件事本身是「好事」，因為通道電荷 Q 變多
- 但同時強垂直電場會把電子 **壓得非常靠近 Si–SiO₂ 界面**，而這個界面並非完美平滑，有粗糙度與缺陷
...
這只是把 $I \propto Q \cdot v$ (或 $Q \cdot \mu$) 用直覺語言重新表達，而非引入新的物理模型。

- $V_{gs} \uparrow \rightarrow Q \uparrow$ （通道電子變多）
- 但同時 $\rightarrow \mu \downarrow$ （電子移動變慢）
...
- **Mobility Degradation**：關鍵量為 $\mu$ ，成因為強垂直電場、界面散射，效果為 $v = \mu E$ 中的「 $\mu$ 變小」
- **Velocity Saturation**：關鍵量為 v，成因為強橫向電場，效果為 v 本身有上限，不再線性成長
...
在 MOS 結構中，gate 與 channel 之間隔著一層氧化層（oxide），其厚度為 $t_{ox}$ 。

當 gate-to-source 電壓 $V_{gs}$ 存在時，在 **垂直方向（由 gate 指向 channel）** 會形成電場：
...
- $V_{gs}$ 越大 → 垂直電場越強 → 被拉進 channel 的電子越多 → 通道中的總電荷量 $Q_{\text{channel}}$ 越大
...
當 drain 與 source 之間存在電壓差 $V_{ds}$ 時，在通道內沿著 **source → drain** 方向會形成橫向電場：
...
在理想 long-channel、low-field 條件下，載子的平均漂移速度可寫為：
...
- **垂直電場 $E_{\text{vert}}$** ：決定通道中有多少電荷 Q，功能是把電子拉進 channel
- **橫向電場 $E_{\text{lat}}$** ：決定電子移動得多快 v，功能是把電子推著沿通道前進
...
- 垂直電場太強 → mobility 下降  
- 橫向電場太強 → 速度飽和  
- 兩種效應都會讓實際電流 **小於理想 long-channel 模型的預測**
...
- $(V_{gs} - V_t - V_{ds}/2)$ ：**平均通道反轉電荷的有效 gate overdrive**
- $V_{ds}$ ：沿通道的橫向推力
...
- **幾何項 W/L** ：設計者能控制  
- **電荷項 $(V_{gs} - V_t - V_{ds}/2)$** ：bulk charge model 已經給你  
- **真正不理想的來源： $\mu$ **  
...
- 問題不在「電流怎麼算」，而在：電子在高電場下跑不動
- 所以後續所有 non-ideal model：mobility degradation、velocity saturation、 $\alpha$ -power law，本質上都是在回答同一件事：
...
本節目的：在維持原本 Linear I–V 架構下，將「高橫向電場時速度不再線性增加」的效應，等效成 ** $\mu$ ** 不再是常數，而是變成一個會隨 $V_{ds}$ 被壓縮的有效遷移率。
...
#### Step 4：把 E 換成 $V_{ds}$ （用 $E_{lat} = V_{ds}/L$ ）

代入 $E = V_{ds}/L$ ：

$$v = \frac{\mu_{\text{eff}} (V_{ds}/L)}{1 + \frac{V_{ds}/L}{E_c}}$$

先把分母整理：

$$\frac{V_{ds}/L}{E_c} = \frac{V_{ds}}{E_c L}$$
...
> **註解（這一步怎麼來的？）**  
> 這裡的想法其實完全來自你前面已經熟到不行的關係： $E_{lat} \approx V_{ds}/L$ 。
> 當橫向電場達到「臨界電場」 $E_c$ 時，代表載子速度即將進入飽和，因此對應的 drain-to-source 電壓自然就是： $V_{ds} = E_c \cdot L$ 。
> 這個「讓橫向電場剛好等於 $E_c$ 的電壓」，我們就定義為 **臨界電壓 $V_c$ **。
> 換句話說： $E_c$ 是材料物理層面的「臨界電場」， $V_c$ 是對應到實際電路端點的「臨界電壓」。
...
$$v = \frac{\mu_{\text{eff}} (V_{ds}/L)}{1 + \frac{V_{ds}}{V_c}} = \left( \frac{\mu_{\text{eff}}}{1 + \frac{V_{ds}}{V_c}} \right) \frac{V_{ds}}{L}$$

到這一步，你可以把它讀成：原本的 $v = \mu \frac{V_{ds}}{L}$ ，現在變成 $v = \mu_{\text{eff,eq}} \frac{V_{ds}}{L}$ ，其中 $\mu_{\text{eff,eq}} = \frac{\mu_{\text{eff}}}{1 + V_{ds}/V_c}$ 。
...
而 Linear I–V 的結構本質是： $I_{ds} \propto (\text{charge term}) \times (\text{drift term})$ 。
...
- 你原本的 Linear I–V 架構不變： $C_{ox} \frac{W}{L} ( \dots ) V_{ds}$
- 真正「被修正」的是 $\mu$ 的表達式：低電場 $\mu$ 當常數，高電場 $\mu \rightarrow \frac{\mu_{\text{eff}}}{1 + V_{ds}/V_c}$
- 也就是：**高 $V_{ds}$ 會讓速度（或等效 $\mu$ ）roll-off，因此電流不再像理想模型那樣隨 $V_{ds}$ 成比例增長。**



### 24.6 速度飽和模型（Velocity Saturation）：從 Square-Law 自然延伸而來的完整推導

> 本節目標：在不推翻 bulk charge 與 square-law 推導骨架的前提下，只修正「載子速度如何隨電場變化」，從而自然導出提早飽和的 $V_{dsat}$ 與非平方律的 $I_{dsat}$ 。
>
> 關鍵觀念：通道電荷怎麼來（ Q = CV ）沒有變，電流定義（ I = Q/t ）沒有變，變的只有「電子實際跑多快」。
...
- 通道在位置 x 的反轉電荷（線電荷密度）： $Q'(x) = C_{ox} W ( V_{gs} - V_t - V(x) )$
- 電流的最原始定義： $I = \frac{Q}{t}$
- 載子穿越通道時間： $t = \frac{L}{v}$
- 因此得到漂移電流形式： $I = Q'(x) \, v(x)$
- 低電場速度模型： $v(x) = \mu E(x) = -\mu \frac{dV}{dx}$

接下來把它 **完整積分一次** （這一步就是 square-law 的核心，過程不省略）。

從 $I = Q'(x) v(x)$ 代入 $Q'(x) = C_{ox} W ( V_{gs} - V_t - V(x) ), \quad v(x) = -\mu \frac{dV}{dx}$ 得到：

$$I = C_{ox} W ( V_{gs} - V_t - V(x) ) \left( -\mu \frac{dV}{dx} \right)$$
...
現在對整條通道積分。邊界條件是 $x : 0 \rightarrow L$ 且 $V : V(0) = 0 \rightarrow V(L) = V_{ds}$ 。因此：

$$\int_{0}^{L} I \, dx = -\mu C_{ox} W \int_{0}^{V_{ds}} ( V_{gs} - V_t - V ) \, dV$$

左邊因為 I 在穩態下沿通道為常數： $\int_{0}^{L} I \, dx = I L$
...
逐項算：
第一項： $\int_{0}^{V_{ds}} ( V_{gs} - V_t ) \, dV = ( V_{gs} - V_t ) V_{ds}$
第二項： $\int_{0}^{V_{ds}} V \, dV = \left. \frac{V^2}{2} \right|_{0}^{V_{ds}} = \frac{V_{ds}^2}{2}$

因此右邊整體為： $\int_{0}^{V_{ds}} ( V_{gs} - V_t - V ) \, dV = ( V_{gs} - V_t ) V_{ds} - \frac{V_{ds}^2}{2}$
...
在 bulk charge / 漸變通道假設下，通道在位置 x 的線電荷密度為： $Q'(x) = C_{ox} W ( V_{gs} - V_t - V(x) )$ 。
飽和（ pinch-off ）邊界的定義是： drain 端的反轉電荷剛好降為 0，也就是在 $x = L$ 時 $Q'(L) = 0$ 。
而 drain 端通道電位 $V(L) = V_{ds}$ ，代入： $0 = C_{ox} W ( V_{gs} - V_t - V_{ds} )$ 。
因此得到飽和邊界電壓： $V_{dsat} = V_{gs} - V_t$ 。

---

#### (b) 把 $V_{ds} = V_{dsat}$ 代回線性區電流式，得到飽和電流 $I_{dsat}$

從線性區電流式出發： $I_{ds} = \beta ( V_{gs} - V_t - V_{ds}/2 ) V_{ds}$ 。
在飽和邊界令 $V_{ds} = V_{dsat} = V_{gs} - V_t$ ，則括號內變成： $V_{gs} - V_t - V_{dsat}/2 = (V_{gs} - V_t)/2$ 。
同時外面的 $V_{ds}$ 也變成 $V_{gs} - V_t$ 。
所以飽和電流為： $I_{dsat} = \mu C_{ox} \frac{W}{L} \frac{( V_{gs} - V_t )^2}{2}$ 。
整理得到你熟悉的 square-law 飽和電流： $I_{dsat} = \frac{1}{2} \mu C_{ox} \frac{W}{L} ( V_{gs} - V_t )^2$ 。

---

#### 24.6.2 為什麼需要速度飽和模型？

上述 square-law 推導隱含一個關鍵假設：電子速度可以隨橫向電場 $E_{lat}$ 無限線性增加。
然而在實際 MOS 中，當通道很短、 $V_{ds}$ 很大、橫向電場過強時，電子會頻繁與晶格散射，導致電子速度不再線性成長，最終趨近一個上限 $v_{sat}$ 。
...
在速度飽和模型中：
- 低電場： $v \approx \mu_{\text{eff}} E$
- 高電場： $v \rightarrow v_{sat}$
常用的一個連續模型寫成： $v(E) = \frac{\mu_{\text{eff}} E}{1 + E/E_c}$
其中 $E_c$ 是臨界電場（critical field），兩者關係為 $E_c = \frac{2v_{sat}}{\mu_{\text{eff}}}$ 。

---

#### 24.6.4 由臨界電場引入「臨界電壓」 $V_c$ 

通道平均橫向電場近似為 $E_{lat} \approx V_{ds}/L$ 。對應臨界電場 $E_c$ 的端點電壓定義為 $V_c \triangleq E_c L$ 。
此 $V_c$ 代表：使整條通道平均橫向電場剛好達到速度飽和門檻的 drain 電壓。

---

#### 24.6.5 修正後的線性區電流（速度被壓縮）

將速度模型改寫為端點電壓形式： $v \approx \frac{\mu_{\text{eff}} (V_{ds}/L)}{1 + V_{ds}/V_c}$ 。
等效上，就是把 square-law 線性區中的 $\mu$ 改為 $\frac{\mu_{\text{eff}}}{1 + V_{ds}/V_c}$ 。
因此得到修正後的線性區電流：

$$I_{ds,\text{lin}} = \frac{\mu_{\text{eff}}}{1 + V_{ds}/V_c} C_{ox} \frac{W}{L} \left( V_{gs} - V_t - \frac{V_{ds}}{2} \right) V_{ds}$$

---

#### 24.6.6 飽和區電流：速度到頂後的物理意義

一旦進入速度飽和區， $v \approx v_{sat}$ 。電流仍由最基本的關係決定： $I = Q'(x) \, v$ 。
在飽和邊界，採用 bulk charge 模型於該關鍵位置： $Q' \approx C_{ox} W ( V_{gs} - V_t - V_{dsat} )$ 。
因此飽和區電流為： $I_{ds,\text{sat}} = C_{ox} W ( V_{gs} - V_t - V_{dsat} ) v_{sat}$ 。

---

#### 24.6.7 由「線性區 = 飽和區」聯立解出新的 $V_{dsat}$ 

在剛進入飽和的交界點： $I_{ds,lin} (V_{ds} = V_{dsat}) = I_{ds,sat}$ 。聯立整理後：

(1) 兩條電流式子在 $V_{ds} = V_{dsat}$ 時相等：

$$\frac{\mu_{\text{eff}}}{1 + V_{dsat}/V_c} C_{ox} \frac{W}{L} \left( V_{gs} - V_t - \frac{V_{dsat}}{2} \right) V_{dsat} = C_{ox} W ( V_{gs} - V_t - V_{dsat} ) v_{sat}$$

(2) 約掉共同因子 $C_{ox} W$ 並利用 $v_{sat} = \frac{\mu_{\text{eff}} V_c}{2 L}$ 代換：

$$\frac{1}{1 + V_{dsat}/V_c} \left( V_{gs} - V_t - \frac{V_{dsat}}{2} \right) V_{dsat} = ( V_{gs} - V_t - V_{dsat} ) \frac{V_c}{2}$$

(3) 令 $V_{ov} = V_{gs} - V_t$ 並展開整理後得到：

$$V_{dsat} ( V_{ov} + V_c ) = V_{ov} V_c \quad \Rightarrow \quad V_{dsat} = \frac{V_{ov} V_c}{V_{ov} + V_c}$$
...
$$\boxed{I_{dsat} = W C_{ox} v_{sat} \frac{(V_{gs} - V_t)^2}{V_{gs} - V_t + V_c}}$$

---

#### 24.6.9 與 Square-Law 的連續性檢查（重要直覺）

當通道很長時（ $V_c \gg V_{gs} - V_t$ ），公式退化為熟悉的 square-law： $I_{dsat} \approx \frac{1}{2} \mu C_{ox} \frac{W}{L} ( V_{gs} - V_t )^2$ 。
當通道很短時（ $V_{gs} - V_t \gg V_c$ ），公式退化為線性依賴： $I_{dsat} \approx W C_{ox} v_{sat} ( V_{gs} - V_t )$ 。

---

#### 24.6.10 本節一句話總結

> **速度飽和模型保留了 bulk charge 與電流定義的骨架，只修正「電子能跑多快」，因而解釋了為何短通道 MOS 的 ON current 不再遵循平方律。**
...
在飽和區時，drain–body PN junction 為反向偏壓，反向偏壓增加會使耗盡區（depletion region）向 source 方向侵入通道。有效導電通道長度變為 $L_{eff} = L - L_d$ 。

---

### 25.2 為何 Ids 會在飽和區隨 Vds 上升？

由於 $I_{ds} \propto W/L$ ，當 $L_{eff}$ 隨 $V_{ds}$ 變小時，等效 $W/L_{eff}$ 增加，即使在飽和區， $I_{ds}$ 仍會隨 $V_{ds}$ 上升。這個現象稱為 **Channel Length Modulation**。

---

### 25.3 工程模型： $\lambda$ 的引入（經驗式修正）

修正後的飽和區電流模型為： $I_{ds} = (\beta / 2) (V_{gs} - V_t)^2 (1 + \lambda V_{ds})$ 。其中 $\lambda$ 為 **channel length modulation coefficient**。

---

### 25.4 $\lambda$ 的物理與工程意義

$\lambda$ 反映飽和區 $I_{ds}$ 對 $V_{ds}$ 的敏感度。通道越長（L 越大）， $\lambda$ 越小，因為耗盡區相對佔比變小。
...
- 本節僅針對 **飽和區模型修正**。
- 物理本質： $V_{ds}$ 增加 → 耗盡區擴張 → 有效通道長度縮短 → $I_{ds}$ 上升。
- 工程處理方式：以 $\lambda$ 將 $V_{ds}$ 的影響封裝進模型。

## 26. Body Effect（體效應）

本節說明：**為何 MOS 電晶體的 threshold voltage（ $V_t$ ）會隨 source–body 電壓改變**。

---

### 26.1 Body 是第四個電晶體端點

Body 並非只是固定接地的背景端點，而是會影響電晶體開關難易度（ $V_t$ ）的關鍵因素。

---

### 26.2 體效應的物理來源（耗盡區觀點）

當 $V_{sb} = V_s - V_b$ 增加時，Source–Body PN junction 的反向偏壓增加，耗盡區變寬，需要更多 gate 電荷才能完成反轉。結果是 **Threshold voltage（ $V_t$ ）上升**。
...
- $V_{t0}$ ： $V_{sb} = 0$ 時的 $V_t$ 。
- $\phi_s$ ：表面位能，摻雜越重 $\phi_s$ 越大，原本就越難反轉。
- $\gamma$ ：體效應係數，描述 $V_{sb}$ 對 $V_t$ 影響的強度。

---

### 26.5 為何體效應公式呈平方根形式？

體效應的本質來自耗盡區行為，而 **耗盡區寬度 $\propto \sqrt{\text{反向偏壓}}$** ，因此呈現平方根關係。

---

### 26.6 NMOS 為何在 source 接地時看不到體效應？

若單顆 NMOS 滿足 $V_s = 0, V_b = 0$ ，則 $V_{sb} = 0$ ，體效應不存在。

---

### 26.7 體效應實際出現的常見情境

只要 source 電位高於 body（如串接 NMOS、Pass transistor），**體效應必然存在**。

---

### 26.8 小 $V_{sb}$ 下的線性近似模型

當 $V_{sb}$ 很小時，可做一階近似： $V_t \approx V_{t0} + k_{\gamma} V_{sb}$ 。

---

### 26.9 線性近似模型的工程意義

提供一個直觀估算方式： $\Delta V_t \approx k_{\gamma} \Delta V_{sb}$ 。

---

### 26.10 Reverse / Forward Body Bias
